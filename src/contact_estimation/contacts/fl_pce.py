...
class pce:
    def __init__(self,topic, batch_size,eval_samples):
        ...
        pub = rospy.Publisher('/fl_contact_state',Float32,queue_size=1)
        ...
        while not rospy.is_shutdown():            
            fl_imu_msg   = rospy.wait_for_message(self.topic, AccelStamped , timeout=None)

            self.data_batch = self.store_new_msg(self.data_batch,fl_imu_msg)
    ...
    def initialize_batch(self):
        ...
        for i in range(self.batch_size):
            ...
            self.data_batch[i,0] =  fl_imu_msg.accel.linear.x
            self.data_batch[i,1] =  fl_imu_msg.accel.linear.y 
            # self.data_batch[i,2] =  fl_imu_msg.accel.linear.z
            # self.data_batch[i,3] =  fl_imu_msg.angular_velocity.x
            # self.data_batch[i,4] =  fl_imu_msg.angular_velocity.y
            # self.data_batch[i,5] =  fl_imu_msg.angular_velocity.z

    def store_new_msg(self,data_batch,fl_imu_msg):

        self.data_batch = np.delete(data_batch,0,axis = 0)
        data_point = np.empty((1,2))
        data_point[0,0] =  fl_imu_msg.accel.linear.x
        data_point[0,1] =  fl_imu_msg.accel.linear.y
        # data_point[0,2] =  fl_imu_msg.accel.linear.z
        # data_point[0,3] =  fl_imu_msg.angular_velocity.x
        # data_point[0,4] =  fl_imu_msg.angular_velocity.y
        # data_point[0,5] =  fl_imu_msg.angular_velocity.z
